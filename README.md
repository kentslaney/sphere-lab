...
immediate release closes the menu. Selecting **debug** toggles grab level curve inspection: while active, a 3D level curve is drawn at the depth of any active grab, with the point along the level curve closest to the grab marked with a small sphere. Config shows the same Depth spread,
